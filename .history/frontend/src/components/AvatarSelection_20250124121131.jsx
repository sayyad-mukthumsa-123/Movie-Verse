...
export default function AvatarSelection({ setSelectedAvatar }) {
  ...
  return (
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {avatars.map((avatarUrl, index) => (
              // <div
              //   key={index}
              //   className={`relative cursor-pointer rounded-lg overflow-hidden ${selectedAvatar === avatarUrl ? "border-4 border-cyan-800" : "border-2 border-gray-300"
              //     }`}
    ...
    ${selectedAvatar === avatarUrl
                    ? "border-8 border-cyan-800"  // Increased border size here
              ...
              >
                <img
                  src={avatarUrl}
                  alt={`avatar-${index}`}
                  className="w-40 h-30  m-1 object-cover transition-transform transform hover:scale-105"
                />
                {selectedAvatar === avatarUrl && (
                  <div className="absolute inset-0 text-xl bg-slate-500 bg-opacity-20 flex justify-center items-center text-white font-bold ">
                    Selected
                  </div>
                )}
              </div>

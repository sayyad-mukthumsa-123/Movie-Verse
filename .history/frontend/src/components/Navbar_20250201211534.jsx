...
const Navbar = ({ onPopularClick, onTrendingClick }) => {
  ...
  useEffect(() => {
    const handleWindowClose = (event) => {
      // Only remove items when the window is truly closed
      if (!event.clientY) {
        localStorage.removeItem('token');
        localStorage.removeItem('email');
      }
    ...
    // Listen to the 'beforeunload' event, which is fired before the page is unloaded
    window.addEventListener('beforeunload', handleWindowClose);

    // Listen to the 'unload' event to ensure it's only cleared on close, not refresh

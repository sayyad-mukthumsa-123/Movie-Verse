...
const FALLBACK_AVATAR = "/fallback-avatar.png"; // Default fallback image
...
export default function AvatarSelection({ setSelectedAvatar }) {
  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/avatars");
        const validAvatars = response.data.filter(avatar => avatar.base64); // Ensure valid avatars
  ...
  const handleAvatarSelect = (avatar) => {
    ...
    setSelectedAvatar(avatar); // Pass full avatar data, not just ID
  };
    ...
    <div className="p-4">
      {isLoading ? (
        ...
        <div>
          <div className="text-2xl text-cyan-800 font-semibold mb-4 text-center">Pick an Avatar</div>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {avatars.map(({ id, base64 }) => (
              <div
                key={id}
                className={`relative cursor-pointer rounded-lg overflow-hidden 
                  ${selectedAvatar?.id === id ? "border-4 border-cyan-800" : "border-2 border-gray-300"}`}
                onClick={() => handleAvatarSelect({ id, base64 })}
              >
                <img
                  src={base64 || FALLBACK_AVATAR} // Ensure fallback is properly used
                  alt="Avatar"

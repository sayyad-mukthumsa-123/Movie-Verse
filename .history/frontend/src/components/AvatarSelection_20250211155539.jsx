...
export default function AvatarSelection({ setSelectedAvatar }) {
  ...
  const handleAvatarSelect = (avatarSvg) => {
    setSelectedAvatarLocal(avatarSvg);
    ...
    <div className="p-4">
      {isLoading ? (
        ...
        <div>
          <div className="text-2xl text-cyan-800 font-semibold mb-4 text-center">Pick an Avatar</div>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {avatars.map((avatarSvg, index) => (
              <div
                key={index}
                className={`relative cursor-pointer rounded-lg overflow-hidden 
                  ${selectedAvatar === avatarSvg ? "border-4 border-cyan-800" : "border-2 border-gray-300"}`}
              ...
              >
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: atob(avatarSvg.split(",")[1]) }}
                />

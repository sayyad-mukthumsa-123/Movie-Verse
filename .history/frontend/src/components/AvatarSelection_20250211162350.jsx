  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      ...
      for (let id of avatarIds) {
        try {
          const response = await axios.get(`http://localhost:5000/api/avatar/${id}`);
          avatarData[id] = response.data;
        } catch (error) {
          console.error(`Failed to fetch avatar ${id}`);
        }
      }
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {avatarIds.map((avatarId) => (
              ...
              >
                <div
                  dangerouslySetInnerHTML={{ __html: avatars[avatarId] }}

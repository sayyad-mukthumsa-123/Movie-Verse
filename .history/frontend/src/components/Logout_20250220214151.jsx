      ...
      {showModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 text-slate-900">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center">
            <p className="text-3xl font-semibold">Are you sure you want to logout?</p>
            <div className="mt-4 flex justify-center gap-4">
              <button
                onClick={handleLogout}
                className="bg-slate-50  text-2xl text-red-500 px-4 py-2 rounded-md outline hover:bg-red-500 hover:text-slate-50"
              >
                Yes, Logout
              </button>
              <button
                onClick={() => setShowModal(false)}
                className="bg-slate-50 text-slate-900 px-4 py-2 rounded-md outline text-2xl hover:bg-slate-900 hover:text-slate-50"
              >
                Cancel
              </button>
            </div>

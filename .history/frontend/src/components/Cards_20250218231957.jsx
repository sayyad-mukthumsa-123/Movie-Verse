...
const Cards = ({ movie }) => {
  ...
  return (
    <div
      className="bg-slate-50 p-4 rounded-lg outline-none cursor-pointer hover:scale-105 outline transition-all flex flex-col justify-between h-[400px]"
      onClick={() => navigate(`/movie/${movie.id}`)} // Navigate to the movie details page
    >
      ...
      <button
        onClick={handleReviewClick}
        className="mt-3 text-cyan-700 outline py-2 px-4 rounded-lg hover:bg-cyan-700 hover:text-white transition-all w-full"
      >
        Review & Rating
      </button>
    </div>

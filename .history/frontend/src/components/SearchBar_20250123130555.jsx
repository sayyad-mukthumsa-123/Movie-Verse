...
const SearchBar = ({ query, setQuery, handleSearch, placeholder }) => {
  return (
    <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4">
      <input
        type="text"
        className="form-control w-full md:w-[500px] lg:w-[600px] xl:w-[700px] px-6 py-3 text-lg border border-gray-800 rounded-md focus:outline-none focus:ring-2 focus:ring-cyan-800"
        id="search-box"
        placeholder={placeholder}
        value={query}
        onChange={(e) => setQuery(e.target.value)}

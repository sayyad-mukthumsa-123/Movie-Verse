...
// Local fallback avatar (add a real fallback image path if needed)
const FALLBACK_AVATAR = "/fallback-avatar.png"; // Place a valid local image in `public/` folder
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {avatarIds.map((avatarId) => (
              <div
                key={avatarId}
                className={`relative cursor-pointer rounded-lg overflow-hidden 
                  ${selectedAvatar === avatarId ? "border-4 border-cyan-800" : "border-2 border-gray-300"}`}
                onClick={() => handleAvatarSelect(avatarId)}
              >
                <img
                  src={`http://localhost:5000/api/avatar/${avatarId}`}
                  alt="Avatar"
                  className="w-40 h-40 p-1 object-cover transition-transform transform hover:scale-105"
                  onError={(e) => {
                    console.error("Avatar failed to load:", avatarId);

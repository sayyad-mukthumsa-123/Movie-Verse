...
const ProfilePage = () => {
  ...
  // Fallback URL for avatar image
  const avatarUrl = user.avatarUrl
    ? `data:image/svg+xml;base64,${user.avatarUrl}` // Avatar from backend (base64 SVG)
    : "https://via.placeholder.com/150"; // Fallback image URL

  // If placeholder is not resolving, we can try using another fallback image URL
  const fallbackAvatarUrl = "https://www.example.com/your-fallback-image.jpg"; // Use a reliable URL
    ...
    <div className="profile-page flex flex-col items-center justify-center min-h-screen bg-slate-200 px-4">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 w-full sm:w-96">
        <div className="flex flex-col items-center mb-6">
          ...
          <img
            src={user.avatarUrl ? avatarUrl : fallbackAvatarUrl} // Use the fallback image URL if avatarUrl is not present or invalid
            alt="User Avatar"
            className="w-28 h-28 rounded-full border border-gray-300 mb-4"
            onError={(e) => (e.target.src = fallbackAvatarUrl)} // Fallback if image fails
          />
          <h2 className="text-xl sm:text-3xl font-semibold text-gray-700 mb-2 sm:mb-3 text-center">
            Username: {user.username}
          </h2>

...
const ReviewRating = () => {
  ...
  useEffect(() => {
    fetchReviews();
    fetchAverageRating();
    fetchUserInfo(); // Fetch the logged-in user's info
  }, [movieId]);
  ...
  const fetchUserInfo = async () => {
    try {
      // Make sure to pass the token to authenticate the request
      const token = localStorage.getItem('token'); // Or use any other method to fetch the user's token
      if (token) {
        const res = await axios.get('http://localhost:5000/api/profile', {
          headers: { Authorization: `Bearer ${token}` },
        });
        setUsername(res.data.username); // Set the username from the profile response
      }
    } catch (error) {

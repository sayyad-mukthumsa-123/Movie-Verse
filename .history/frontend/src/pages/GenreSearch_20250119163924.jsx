...
const GenreSearch = () => {
    ...
    const handleSearch = async (e) => {
        ...
        if (!GENRE_MAP[genre.toLowerCase()]) {
            setError('Please enter a valid genre (e.g., Action, Comedy, Drama, etc.)');
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            const teluguMovies = moviesData.filter(movie => movie.original_language === 'te').slice(0, 5);
            const hindiMovies = moviesData.filter(movie => movie.original_language === 'hi').slice(0, 5);
            const englishMovies = moviesData.filter(movie => movie.original_language === 'en').slice(0, 5);

            const filteredMovies = [...teluguMovies, ...hindiMovies, ...englishMovies];

            if (filteredMovies.length > 0) {
                setMovies(filteredMovies);
                setError('');
            } else {
                setError('No movies found for the selected genre and languages.');
                setMovies([]);
            }
        } catch (err) {

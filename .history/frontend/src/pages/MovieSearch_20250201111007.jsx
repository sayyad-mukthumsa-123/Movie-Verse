...
const Movie = () => {
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=${tmdbApiKey}&query=${query}&language=en-US`
      );
      if (response.data.results.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      <h1 className="text-4xl font-bold text-cyan-800 text-center mb-8">Search Movies</h1>
      <SearchBar
        query={query}
        setQuery={setQuery}
      ...
      <div className="w-full  p-4 mt-8">
        {movies.length > 0 ? (
          ...
          !loading && (
            <p className="text-lg text-gray-600 text-center mt-4">No movies found. Try searching for another movie!</p>
          )
        )}
      </div>
    </div>

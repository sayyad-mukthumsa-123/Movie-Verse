...
const Profile = () => {
  ...
  const [isEditing, setIsEditing] = useState(false);
  const [updatedData, setUpdatedData] = useState({});
  const [isLoggedOut, setIsLoggedOut] = useState(false); // Added to track logout status

  useEffect(() => {
    const token = localStorage.getItem('token');
    if (!token) {
      setIsLoggedOut(true); // Mark the user as logged out if token is missing
      return;
    }
    ...
    fetchProfileData(); // Fetch profile data when the component is mounted
  }, [isLoggedOut]); // Re-run effect when logged out
  ...
  const handleLogout = () => {
    ...
    setIsLoggedOut(true); // Mark as logged out so that profile clears
  };

  if (isLoggedOut) {
    return <div>You have been logged out. Please log in again.</div>; // Show message if logged out
  }

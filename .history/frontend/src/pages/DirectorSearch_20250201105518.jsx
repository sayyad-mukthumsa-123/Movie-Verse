...
const DirectorSearch = () => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            const personResponse = await axios.get(`${API_URL}/search/person`, {
                params: { api_key: API_KEY, query: director },
            });
    ...
    return (
        <>
            <h1 className="text-4xl font-bold text-center text-cyan-800 mb-8">Search By Director</h1>
            <SearchBar query={director} setQuery={setDirector} handleSearch={handleSearch} placeholder="Enter director name" />
            {loading && <p className="text-xl text-gray-700 mt-4">Loading...</p>}
            {error && <p className="text-red-500 text-lg mt-4">{error}</p>}
            <div className="movies-container grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 p-4 mt-8">
                {movies.length > 0 ? (
                    movies.map((movie) => (
                        <Cards key={movie.id} movie={movie} onClick={handleMovieClick} />
                    ))

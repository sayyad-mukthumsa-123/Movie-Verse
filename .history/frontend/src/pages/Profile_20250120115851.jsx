...
const Profile = () => {
  ...
  const decoded = token ? jwtDecode(token) : null; // Use jwtDecode to decode the token
  const userId = decoded ? decoded.user.id : null;

  useEffect(() => {
    if (userId) {
      // Fetch user profile
      axios
        .get(`/api/profile/${userId}`, { headers: { "x-token": token } }) // Update URL with userId
        .then((response) => setProfile(response.data))
        .catch((error) => console.error("Error fetching profile:", error));
    }
  }, [userId, token]);
  ...
  const handleSave = () => {
    axios
      .put(`/api/profile/${userId}`, profile, { headers: { "x-token": token } }) // Update URL with userId
      .then((response) => {
        alert("Profile updated successfully");
        setIsEditing(false);
        setProfile(response.data.user);

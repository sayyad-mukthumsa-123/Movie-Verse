...
const Profile = () => {
  ...
  // Extract userId from URL using useParams hook
  const { userId } = useParams();

  useEffect(() => {
    if (userId) {
      // Fetch user profile using the userId from the URL
      const token = localStorage.getItem("token"); // Retrieve token for authentication
      axios
        .get(`/api/profile/${userId}`, { headers: { "x-token": token } })
        .then((response) => setProfile(response.data))
        .catch((error) => console.error("Error fetching profile:", error));
    }
  }, [userId]);
  ...
  const handleSave = () => {
    const token = localStorage.getItem("token"); // Retrieve token for authentication
    axios
      .put(`/api/profile/${userId}`, profile, { headers: { "x-token": token } })
      .then((response) => {
        alert("Profile updated successfully");
        setIsEditing(false);
        setProfile(response.data.user);

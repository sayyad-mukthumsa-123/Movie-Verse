...
const ReviewRating = () => {
  ...
  const handleDelete = async (reviewId, reviewUsername) => {
    ...
    if (confirmUsername !== reviewUsername) {
      alert("Username does not match. Review not deleted.");
      return;
    }
    ...
    try {
      ...
      // Refresh reviews and average rating after deletion
      fetchReviews();
      fetchAverageRating();
    } catch (error) {
      console.error('Error deleting review:', error.response?.data?.message || 'Error');
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mt-8">
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div key={review._id} className="flex justify-between items-center mb-6 p-4 bg-gray-50 rounded-lg shadow-md">
              <div>
                <p className="text-lg font-semibold text-cyan-800">
                  <strong>{review.username}</strong> ({review.rating} ⭐)
                </p>

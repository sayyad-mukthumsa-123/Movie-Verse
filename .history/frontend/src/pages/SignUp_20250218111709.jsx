...
const Signup = () => {
  ...
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [selectedAvatar, setSelectedAvatar] = useState(""); // Ensure you have a default avatar URL or handle this input
  const navigate = useNavigate();

  // Handle file upload for avatar (optional)
    ...
    try {
      const response = await fetch("http://localhost:5000/register", {
        ...
        body: JSON.stringify({
          ...
          Password: password,
          ConfirmPassword: confirmPassword,
          Avatar: selectedAvatar, // Send selected avatar (base64 or URL)
        }),
      });

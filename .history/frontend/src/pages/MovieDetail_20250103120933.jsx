...
//         const recommendationResponse = await axios.get(`/api/recommend/${id}`);
...
//               <li key={rec.tmdbId}>{rec.title}</li>
//             ))}
//           </ul>
//         </div>
//       )}
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        ...
        fetchTrailer(id);
        fetchCast(id);
        fetchRecommendations(director.name); // Fetch recommendations based on director
      } catch (err) {
        setError('An error occurred while fetching the movie details.');
      } finally {
        setLoading(false);
    ...
    const fetchRecommendations = async (directorName) => {
      try {

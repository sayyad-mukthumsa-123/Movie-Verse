...
const ProfilePage = () => {
  ...
  const getAvatar = () => {
    if (user?.avatarUrl) {
      return user.avatarUrl; // Use stored avatar from the database
    }
    return `data:image/svg+xml;utf8,${encodeURIComponent(multiavatar(email))}`; // Use email for consistent avatar
  };

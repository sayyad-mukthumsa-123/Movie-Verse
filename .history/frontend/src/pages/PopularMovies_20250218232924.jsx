...
const PopularMovies = () => {
    ...
    const fetchMovies = async () => {
      try {
        const [teluguResponse, hindiResponse, englishResponse] = await Promise.all([
          ...
          fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=en-US&sort_by=vote_average.asc&vote_count.gte=130&with_original_language=en&page=1`),
        ]);
        ...
        const englishMovies = englishData.results.slice(0, 18);
  ...
  return (
    <div className="bg-slate-900 h-fit"> 
      <h1 className="text-4xl  font-bold text-center text-white p-4">Popular Movies</h1>

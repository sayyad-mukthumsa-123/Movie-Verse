...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (response.data) {
          setUser({ ...response.data }); // Ensures a fresh re-render
        } else {
          setError("User not found");
        }
      } catch (err) {
  ...
  console.log("Fetched Avatar URL:", user.avatarUrl);
    ...
    <div className="profile-page flex flex-col items-center justify-center min-h-screen bg-slate-200 px-4">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 w-full sm:w-96">
        <div className="flex flex-col items-center mb-6">
          {user.avatarUrl ? (
            <img
              src={`${user.avatarUrl}?t=${Date.now()}`} // Prevents caching issues
              alt="User Avatar"
              className="w-20 h-20 sm:w-28 sm:h-28 rounded-full border border-gray-300 mb-4"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = "/default-avatar.png"; // Fallback image if URL is broken
              }}
            />
          ) : (

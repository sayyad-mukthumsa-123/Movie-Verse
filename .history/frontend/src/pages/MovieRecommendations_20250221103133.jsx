    ...
    <div className="p-4 outline rounded-lg">
      <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {recommendations.map((movie) => (
          // <li
          //   key={movie.id}
          //   className="bg-gray-800 p-4 rounded-lg shadow-lg outline hover:bg-gray-700 transition cursor-pointer"
          //   onClick={() => navigate(`/movie/${movie.id}`)}
          // >
          <li
            key={movie.id}
            className="bg-gray-800 p-4 rounded-lg shadow-lg outline hover:bg-gray-700 transition cursor-pointer"
            onClick={() => handleMovieClick(movie.id)} // Use handleMovieClick function
          >

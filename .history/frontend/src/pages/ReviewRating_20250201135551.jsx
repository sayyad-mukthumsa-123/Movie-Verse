...
const ReviewRating = () => {
  ...
  const handleDelete = async (reviewId) => {
    const token = localStorage.getItem('token');
    const username = localStorage.getItem('username'); // Retrieve the username from localStorage or state
  
    console.log("Token:", token); // Log token for debugging
    console.log("Username:", username); // Log username for debugging
  
    try {
      // Sending the DELETE request with the token in the header and username in the body
      const response = await axios.delete(`http://localhost:5000/api/reviews/${reviewId}`, {
        headers: {

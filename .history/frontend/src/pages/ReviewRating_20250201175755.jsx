...
const ReviewRating = () => {
  ...
  const handleDelete = async (reviewId, reviewUsername) => {
    const token = localStorage.getItem('token');
    const userInput = prompt("Please enter your username to confirm deletion:");

    if (userInput !== reviewUsername) {
      alert("Username does not match. Review deletion canceled.");
      return;
    }
    ...
    try {
      await axios.delete(`http://localhost:5000/api/reviews/${reviewId}`, {
        ...
        data: {
          username: reviewUsername, // Pass the username to validate the deletion
        },
      });

      // Refresh the reviews and average rating after deleting
      fetchReviews();
      fetchAverageRating();
    } catch (error) {
      console.error('Error deleting review:', error.response?.data?.message || 'Error');
      ...
      <div className="mt-8">
        ...
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div
              key={review._id}
              className="mb-6 p-4 bg-gray-50 rounded-lg shadow-md flex justify-between items-center"
            >
              <div>
                <p className="text-lg font-semibold text-cyan-800">
                  <strong>{review.username}</strong> ({review.rating} ⭐)

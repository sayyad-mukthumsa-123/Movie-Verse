...
//             <div className="w-full outline-dashed max-w-screen-lg mt-8">
//                 {movies.length > 0 ? (
...
const DirectorSearch = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
            ...
            {loading && <p className="text-xl text-gray-700 mt-4">Loading...</p>}
            {error && <p className="text-red-500 text-lg mt-4">{error}</p>}
            <div className="w-full outline-dashed max-w-screen-xl mt-8  gap-2" > {/* Increased width from 'max-w-screen-lg' to 'max-w-screen-xl' */}
                {movies.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6"> {/* Increased grid column count for larger screens */}
                        {movies.map((movie) => (
                            <Cards key={movie.id} movie={movie} onClick={handleMovieClick} />
                        ))}
                    </div>

...
const ReviewRating = () => {
  ...
  useEffect(() => {
    fetchReviews();
    fetchAverageRating();
    fetchUserInfo(); // Fetch user info based on email
  }, [movieId]);
...
//         const email = res.data.email; // Assuming email is available from the profile
//         const profileRes = await axios.get(`http://localhost:5000/api/profile/${email}`);
//         setUsername(profileRes.data.username); // Set username from the email-based profile route
//       }
//     } catch (error) {
...
const fetchUserInfo = async () => {
    try {
      ...
      if (token) {
        ...
        const email = res.data.email; // Assuming email is available from the profile
        const profileRes = await axios.get(`http://localhost:5000/api/profile/${email}`); // Correct endpoint with email
        setUsername(profileRes.data.username); // Set username from the email-based profile route
      }
    } catch (error) {
      console.error('Error fetching user info:', error.response?.data?.message || 'Error');

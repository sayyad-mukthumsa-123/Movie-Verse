...
//     res.status(201).json({ message: "Review submitted successfully", review: reviewDoc });
//   } catch (err) {
...
app.post("/api/reviews/:movieId", middleware, async (req, res) => {
  const { movieId } = req.params; // Fix here, use movieId directly
  const { review } = req.body;
  ...
  try {
    const reviewDoc = new ReviewModel({
      ...
      userId: req.user.id,
      username: req.user.username,
      text: review.trim(), // Ensure the text is trimmed before saving
    });

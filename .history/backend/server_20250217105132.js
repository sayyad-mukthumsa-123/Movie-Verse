...
// Avatar API: Generate a list of avatars using AvatarAPI
// app.get("/api/avatars", async (req, res) => {
//   try {
//     const avatars = [];
//     for (let i = 0; i < 4; i++) {
//       const avatarId = Math.random().toString(36).substring(7);
//       const response = await axios.get(`https://api.avatarapi.com/avatar/${avatarId}`);
//       avatars.push({ id: avatarId, avatarUrl: response.data.avatarUrl });
//     }
//     res.status(200).json(avatars);
//   } catch (error) {
//     console.error("Error generating avatars:", error.message);
//     res.status(500).json({ error: "Failed to generate avatars. Please try again later." });
//   }
// });
...
//       throw new Error("Avatar URL not found in the response.");
//     }
...
app.get("/api/avatars", async (req, res) => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (!response.data || !response.data.avatarUrl) {
        throw new Error(`Avatar URL not found for avatarId: ${avatarId}`);
      }

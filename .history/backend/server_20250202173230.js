...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    ...
    // Fetch movies with similar genres for different languages (Telugu, Hindi, English)
    const teluguMovies = await fetchMoviesByGenreAndLanguage(genreIds, "te"); // Telugu
    const hindiMovies = await fetchMoviesByGenreAndLanguage(genreIds, "hi"); // Hindi
    const englishMovies = await fetchMoviesByGenreAndLanguage(genreIds, "en"); // English

    // Filter out the current movie from recommendations
    const allRecommendedMovies = [
      ...teluguMovies.slice(0, 5), // Top 5 Telugu movies
      ...hindiMovies.slice(0, 5),  // Top 5 Hindi movies
      ...englishMovies.slice(0, 5) // Top 5 English movies
    ];

    // Remove the current movie from the recommendations list
    const filteredRecommendations = allRecommendedMovies.filter(movie => movie.id !== movieId);

...
app.get("/api/avatars", async (req, res) => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      const randomSeed = Math.random().toString(36).substring(7); // Generate a random string
      const response = await axios.get(`https://api.multiavatar.com/${randomSeed}.svg`, {
        responseType: "text", // Fetch as text (SVG)
      });

      const svgBase64 = Buffer.from(response.data).toString("base64"); // Convert to Base64
      avatars.push(`data:image/svg+xml;base64,${svgBase64}`);
    }

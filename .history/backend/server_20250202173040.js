...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    ...
    // Fetch movies in different languages: Telugu, Hindi, English
    const languages = ['te', 'hi', 'en']; // Telugu, Hindi, English

    // Fetch recommendations from each language
    const recommendations = {};
    for (let lang of languages) {
      recommendations[lang] = await fetchMoviesByGenreAndLanguage(genreIds, lang);
    }

    // Flatten the results, limit to 5 per language, and filter out the current movie
    const finalRecommendations = {
      telugu: recommendations.te.slice(0, 5).filter(movie => movie.id !== movieId),
      hindi: recommendations.hi.slice(0, 5).filter(movie => movie.id !== movieId),
      english: recommendations.en.slice(0, 5).filter(movie => movie.id !== movieId),
    };

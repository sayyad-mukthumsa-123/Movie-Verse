...
async function fetchMoviesByLanguage(language, limit = 5) {
  try {
    const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_original_language=${language}`);
    return response.data.results.slice(0, limit); // Limit to specified number of movies
  } catch (error) {
    return [];
  }
}
...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    ...
    // Fetch 5 movies in each language (Telugu, Hindi, English)
    const teluguMovies = await fetchMoviesByLanguage("te");
    const hindiMovies = await fetchMoviesByLanguage("hi");
    const englishMovies = await fetchMoviesByLanguage("en");

    // Combine all the fetched movies into one list
    ...
    // Filter out the current movie from recommendations
    const filteredRecommendations = combinedRecommendations.filter(movie => movie.id !== movieId);

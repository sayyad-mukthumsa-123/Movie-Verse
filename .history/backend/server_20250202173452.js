...
async function fetchMoviesByGenreAndLanguage(genreIds, languages) {
  try {
    const promises = languages.map(async (language) => {
      const genrePromises = genreIds.map(async (genreId) => {
        const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&with_original_language=${language}`);
        return response.data.results;
      });

      const genreMovies = await Promise.all(genrePromises);
      return genreMovies.flat().slice(0, 5);  // Limit to 5 movies per language
    });

    const moviesByLanguage = await Promise.all(promises);
    return moviesByLanguage.flat();
  } catch (error) {
    return [];
  }
}
...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    ...
    // Define the languages (Telugu, Hindi, and English)
    const languages = ['te', 'hi', 'en'];

    // Fetch movies with similar genres and languages
    const recommendedMovies = await fetchMoviesByGenreAndLanguage(genreIds, languages);

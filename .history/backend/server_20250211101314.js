...
// API to fetch avatars
app.get("/api/avatars", async (req, res) => {
  try {
    const avatars = [];
    for (let i = 0; i < 4; i++) {
      const randomId = Math.floor(Math.random() * 1000); // Generate a random avatar ID
      const avatarUrl = `https://api.multiavatar.com/${randomId}.svg`;

      // Fetch the avatar as raw image data
      const response = await axios.get(avatarUrl, { responseType: "arraybuffer" });

      // Convert to base64 format
      const base64Avatar = Buffer.from(response.data, "binary").toString("base64");

      // Format as Data URL
      avatars.push(`data:image/svg+xml;base64,${base64Avatar}`);
    }

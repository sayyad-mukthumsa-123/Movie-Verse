  ...
  try {
    const promises = genreIds.map(async (genreId) => {
      const response = await axios.get(
        `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&language=${language}`
      );
      return response.data.results;
    });
...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    ...
    // Fetch movies for each language (Telugu, Hindi, English)
    const teluguMovies = await fetchMoviesByGenreAndLanguage(genreIds, 'te');
    const hindiMovies = await fetchMoviesByGenreAndLanguage(genreIds, 'hi');
    const englishMovies = await fetchMoviesByGenreAndLanguage(genreIds, 'en');

    // Combine all the movies and remove duplicates by movie ID
    const allMovies = [...teluguMovies, ...hindiMovies, ...englishMovies];
    const uniqueMovies = allMovies.filter((value, index, self) =>
      index === self.findIndex((t) => t.id === value.id)
    );

    // Filter out the current movie from recommendations
    const filteredRecommendations = uniqueMovies.filter(movie => movie.id !== movieId);

    // Limit to the top 15 recommended movies (5 from each language)
    const topTeluguMovies = teluguMovies.slice(0, 5);
    const topHindiMovies = hindiMovies.slice(0, 5);
    const topEnglishMovies = englishMovies.slice(0, 5);

    const topRecommendations = [...topTeluguMovies, ...topHindiMovies, ...topEnglishMovies];

  ...
  try {
    const avatars = await Promise.all(
      Array.from({ length: 4 }, async () => {
        ...
        try {
          const response = await axios.get(avatarUrl, { responseType: "arraybuffer" });
          const base64Avatar = Buffer.from(response.data, "binary").toString("base64"); // Correct encoding
          return { id: avatarId, base64: `data:image/svg+xml;base64,${base64Avatar}` };
        } catch (error) {
          console.error(`⚠️ Error fetching avatar for ID ${avatarId}:`, error.message);
          return null; // Skip failed fetches
        }
      })
    );

    res.json(avatars.filter(Boolean)); // Remove null values
  } catch (error) {
    console.error("❌ Error generating avatars:", error.message);
    res.status(500).json({ error: "Failed to generate avatars" });
  }

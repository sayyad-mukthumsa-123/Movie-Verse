...
const middleware = (req, res, next) => {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    req.user = decoded; // The decoded token now includes the email
    next();
  } catch (error) {
    return res.status(400).json({ msg: "Invalid token" });
  }
...
// Fetch user profile based on email (from token)
app.get("/api/profile", middleware, async (req, res) => {
  try {
    const user = await UserModel.findOne({ email: req.user.email }).select("-Password");
    if (!user) return res.status(404).json({ msg: "User not found." });
    res.json(user); // Sends back the user data
  } catch (error) {
    console.error("Error fetching profile:", error);
    res.status(500).json({ msg: "Server error." });
  }
...
// Update user profile based on email (from token)
app.put("/api/profile", middleware, async (req, res) => {
  try {
    const { Username, Password, Avatar } = req.body;
    const updates = {};

...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    const targetMovie = await fetchMovieDetails(movieId);
    if (!targetMovie) return res.status(500).json({ error: "Failed to fetch movie data." });

    const genreIds = targetMovie.genres.map((genre) => genre.id);
    const recommendedMovies = await fetchMoviesByGenre(genreIds);
    const filteredRecommendations = recommendedMovies.filter((movie) => movie.id !== parseInt(movieId));

    res.json(filteredRecommendations.slice(0, 10)); // Return top 10 recommendations
  } catch (error) {
    res.status(500).json({ error: "Internal server error" });
  }
});

// Fetch Exactly 4 Avatars
app.get("/api/avatars", async (req, res) => {
  try {
    const avatars = Array.from({ length: 4 }, () => Math.random().toString(36).substring(7));
    res.status(200).json(avatars);
  } catch (error) {
    console.error("Error fetching avatar IDs:", error.message);

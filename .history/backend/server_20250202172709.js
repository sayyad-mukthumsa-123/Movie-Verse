...
// Function to fetch movies by genre and filter by languages (Telugu, Hindi, and English)
async function fetchMoviesByGenre(genreIds, languages = ['en', 'hi', 'te']) {
  try {
    // Convert the languages array into a comma-separated string
    const languageString = languages.join(',');

    const promises = genreIds.map(async (genreId) => {
      // Make the API request to filter movies by both genre and language
      const response = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_genres=${genreId}&with_original_language=${languageString}`);
      return response.data.results;
    });
...
// API Route to recommend movies based on the current movie's genre and language
app.get("/api/recommendations/:movieId", async (req, res) => {
  const { movieId } = req.params;
  ...
  try {
    ...
    // Fetch movies with similar genres and languages (Telugu, Hindi, English)
    const recommendedMovies = await fetchMoviesByGenre(genreIds);

...
app.get("/api/avatars", async (req, res) => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      const avatarId = Math.random().toString(36).substring(7); // Generate unique avatar ID
...
app.get("/api/avatar/:id", async (req, res) => {
  try {
    ...
    res.status(200).send(response.data); // Return raw SVG data
  } catch (error) {

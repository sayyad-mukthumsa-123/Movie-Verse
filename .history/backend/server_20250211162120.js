...
app.get("/api/avatars", async (req, res) => {
  try {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const avatarUrl = `https://api.multiavatar.com/${randomSeed}.svg`;

      // Fetch avatar SVG from Multiavatar API
      const response = await axios.get(avatarUrl, { responseType: "text" });

      avatars.push({
        seed: randomSeed,
        svg: response.data, // SVG content
      });
    }

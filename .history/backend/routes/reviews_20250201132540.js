...
//     try {
//         const review = await Review.findById(req.params);
...
// router.delete('/:reviewId', authMiddleware, async (req, res) => {
//   const { reviewId } = req.params; // Accessing the reviewId from URL parameters
//   console.log("Received reviewId:", reviewId); // Log to verify if it's received
...
// console.log("Logged-in Username:", req.user);
...
router.delete('/:reviewId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Fetch user by ID from the database using req.user.id
    const user = await User.findById(req.user.id); // Assuming req.user.id contains the logged-in user's ID
    if (!user) return res.status(404).send('User not found');

    // Compare the review's username with the logged-in user's username
    if (review.username !== user.Username) {  // If they don't match, deny deletion
      console.log("Review Username:", review.username);
    ...
    // If authorized, remove the review
    await review.remove();
    res.status(200).send('Review deleted');
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });

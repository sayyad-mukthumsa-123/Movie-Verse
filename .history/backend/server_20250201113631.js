...
app.post('/', async (req, res) => {
  try {
    const { movieId, userId, username, rating, reviewText } = req.body;

    if (!movieId || !userId || !rating || !reviewText) {
      return res.status(400).json({ message: 'All fields are required' });
    }

    const newReview = new Review({ movieId, userId, username, rating, reviewText });
...
app.get('/:movieId/average-rating', async (req, res) => {
  try {

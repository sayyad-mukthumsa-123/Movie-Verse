...
app.get("/api/recommendations/:movieId", async (req, res) => {
  ...
  try {
    // Fetch the current movie's details including genres and language
    const targetMovie = await fetchMovieDetails(movieId);
    ...
    // Fetch movies with similar genres for different languages
    const languages = ['te', 'hi', 'en'];  // Telugu, Hindi, English
    const languageMovies = {};

    for (const language of languages) {
      const recommendedMovies = await fetchMoviesByGenreAndLanguage(genreIds, language);
      languageMovies[language] = recommendedMovies;
    }

    // Combine and flatten the results, ensuring we have 5 movies from each language
    const topMovies = [];
    for (const language of languages) {
      topMovies.push(...languageMovies[language].slice(0, 5));
    }

    // Filter out the current movie from recommendations and limit to 15
    const filteredRecommendations = topMovies.filter(movie => movie.id !== movieId).slice(0, 15);

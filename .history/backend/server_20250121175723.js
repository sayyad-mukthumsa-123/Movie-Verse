...
app.get("/profile", middleware, async (req, res) => {
  try {
    const userEmail = req.user.email; // Retrieve email from decoded token
    if (!userEmail) {
      return res.status(400).json({ msg: "User email is missing in token." });
    }

    const user = await UserModel.findOne({ Email: userEmail }).select("-Password"); // Exclude password field
    ...
    res.status(200).json({
      username: user.Username,
      email: user.Email,
      avatar: user.Avatar || "", // Provide fallback for missing avatar
    });
  } catch (error) {

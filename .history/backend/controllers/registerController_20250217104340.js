...
const Register = async (req, res) => {
  try {
    ...
    // Avatar logic (fetch from AvatarAPI if not provided)
    let avatarUrl = Avatar;
    if (!avatarUrl) {
      const response = await axios.get(`https://api.avatarapi.com/avatar/${Username}`);
      avatarUrl = response.data.avatarUrl; // Assuming AvatarAPI returns a URL
    }
